...
// ---------------------------------------------------------------------------
// Where saved configurations live. "Our database for now" = Netlify Blobs —
// it needs no new account, no connection string, nothing beyond what you
// already have with Netlify. getStore() auto-connects to this site because
// it's running inside a Netlify Function; no config object needed.
//
// TO SWITCH DATABASES LATER: replace the three functions below
// (saveConfiguration / listConfigurations / getConfiguration) with calls to
// whatever you're moving to. Nothing else in this file, or in index.html,
// needs to change — they only ever call these three functions.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Same session check as gate.ts, kept independent (not imported from gate.ts)
// so nothing here can break the login flow that's already working. A little
...
function secret() {
  return String(Netlify.env.get("SESSION_SECRET") || Netlify.env.get("CUSTOMER_ACCESS_CODE") || "ds-session");
}
...
export const config: Config = { path: "/api/configurations" };
